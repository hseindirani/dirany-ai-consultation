using DiranyAI.Api.AI;
using DiranyAI.Api.Consultations.Dtos;
using DiranyAI.Api.Data;
using DiranyAI.Api.Storage;
using Microsoft.EntityFrameworkCore;

namespace DiranyAI.Api.Consultations;

public class CombinedPreviewService
{
    private readonly AppDbContext _dbContext;
    private readonly IImageStorage _imageStorage;
    private readonly IImageGenerationService _imageGenerationService;

    public CombinedPreviewService(
        AppDbContext dbContext,
        IImageStorage imageStorage,
        IImageGenerationService imageGenerationService)
    {
        _dbContext = dbContext;
        _imageStorage = imageStorage;
        _imageGenerationService = imageGenerationService;
    }

    public async Task<CombinedPreviewResponse> GenerateAsync(
        long consultationId,
        CancellationToken cancellationToken = default)
    {
        var selectedHairCandidate = await _dbContext.HairCandidates
            .Include(c => c.HairStyle)
            .FirstOrDefaultAsync(
                c => c.ConsultationId == consultationId &&
                     c.IsSelected,
                cancellationToken);

        if (selectedHairCandidate is null)
        {
            throw new ArgumentException(
                "A hair candidate must be selected before generating a combined preview.");
        }

        var selectedBeardCandidate = await _dbContext.BeardCandidates
            .Include(c => c.BeardStyle)
            .FirstOrDefaultAsync(
                c => c.ConsultationId == consultationId &&
                     c.IsSelected,
                cancellationToken);

        if (selectedBeardCandidate is null)
        {
            throw new ArgumentException(
                "A beard candidate must be selected before generating a combined preview.");
        }

        var originalImage = await _dbContext.ConsultationImages
            .FirstOrDefaultAsync(
                i => i.ConsultationId == consultationId &&
                     i.ImageType == ConsultationImageType.Original,
                cancellationToken);

        if (originalImage is null)
        {
            throw new ArgumentException(
                "This consultation does not have an original image.");
        }

        await using var originalImageStream =
            await _imageStorage.OpenReadAsync(
                originalImage.StoragePath,
                cancellationToken);

        var prompt =
            $"Edit the hairstyle to: {selectedHairCandidate.HairStyle.AiPromptHint}. " +
            $"Edit the beard to: {selectedBeardCandidate.BeardStyle.AiPromptHint}. " +
            "Keep the same person, facial features, skin tone, pose, lighting, clothing, and background. " +
            "Change only the hairstyle and beard and make the result photorealistic.";

        var fileName = Path.GetFileName(originalImage.StoragePath);

        var contentType = Path.GetExtension(fileName).ToLowerInvariant() switch
        {
            ".jpg" or ".jpeg" => "image/jpeg",
            ".png" => "image/png",
            ".webp" => "image/webp",
            _ => throw new ArgumentException("Unsupported original image format.")
        };

        var imageInput = new ImageInput
        {
            Stream = originalImageStream,
            FileName = fileName,
            ContentType = contentType
        };

        await using var generatedImageStream =
            await _imageGenerationService.GenerateCombinedPreviewAsync(
                imageInput,
                prompt,
                cancellationToken);

        var previewFileName =
            $"combined-preview-{consultationId}.png";

        var storagePath =
            await _imageStorage.SaveAsync(
                generatedImageStream,
                previewFileName,
                cancellationToken);

        var previewImage = new ConsultationImage
        {
            ConsultationId = consultationId,
            HairCandidateId = selectedHairCandidate.Id,
            BeardCandidateId = selectedBeardCandidate.Id,
            ImageType = ConsultationImageType.CombinedPreview,
            StoragePath = storagePath,
            CreatedAt = DateTimeOffset.UtcNow
        };

        _dbContext.ConsultationImages.Add(previewImage);

        await _dbContext.SaveChangesAsync(cancellationToken);

        return new CombinedPreviewResponse
        {
            ImageId = previewImage.Id
        };
    }
}