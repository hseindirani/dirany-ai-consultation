...
public class AzureImageGenerationService : IImageGenerationService
{
    ...
    public async Task<Stream> GenerateBeardPreviewAsync(
    ImageInput originalImage,
    string prompt,
    CancellationToken cancellationToken = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException(
                $"Azure OpenAI beard image edit failed with status {(int)response.StatusCode}: {errorBody}");
        }

        var responseBody =
            await response.Content.ReadFromJsonAsync<ImageEditResponse>(
                cancellationToken);

        var base64Image =
            responseBody?.Data.FirstOrDefault()?.Base64Json;

        if (string.IsNullOrWhiteSpace(base64Image))
        {
            throw new InvalidOperationException(
                "Azure OpenAI did not return a generated beard image.");
        }
